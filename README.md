# TeleHealings

Telehealth marketplace platform built with Expo (mobile), React/Vite (web), and NestJS (backend).

## Status
- **Backend**: Deployed on Railway
- **Admin Portal**: https://telehealings-admin.vercel.app
- **Therapist Portal**: https://telehealings-therapist.vercel.app
- **API Docs**: Available at /api/docs (Swagger UI)

## Setup
```bash
# Install dependencies
npm install

# Start all services
npm run dev

# Start individual packages
npm run dev:admin    # Admin portal on port 5174
npm run dev:therapist # Therapist portal on port 5173
npm run dev:backend   # Backend API on port 5172
npm run dev:mobile   # Mobile app on port 3001
```
