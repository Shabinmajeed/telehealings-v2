...
export default function PersonalisationScreen() {
  ...
  const toggleOption = (id: string) => {
    setSelected((prev) => {
      ...
      if (next.size >= MAX_SELECTIONS) { setErrorText('You can only choose a maximum of 3 cards.'); return prev; }
      next.add(id); setErrorText(''); return next;
    });
  };

  const handleContinue = async () => {
    if (selected.size === 0) { setErrorText('Select at least 1 option to continue'); return; }
    const personalisation = Array.from(selected);
    try {
  ...
  return (
    <View style={styles.container}>
      <Animated.View style={[styles.topCurve, slideFade(headerAnim)]}>
        <LinearGradient colors={['#cbe0f9', '#e2effa']} start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }} style={StyleSheet.absoluteFillObject} />
        ...
        <Svg height={25} width="100%" viewBox={`0 0 ${SCREEN_WIDTH} 25`} preserveAspectRatio="none" style={styles.curveSvg}>
      ...
      <View style={styles.contentSection}>
        <Animated.View style={[styles.optionsGrid, slideFade(gridAnim)]}>
          {OPTIONS.map((option) => {
            ...
            return (
              <TouchableOpacity key={option.id} style={[styles.optionCard, isSelected && styles.optionCardSelected]} onPress={() => toggleOption(option.id)} activeOpacity={0.8}>
      ...
      <Animated.View style={[styles.bottomSection, slideFade(bottomAnim)]}>
        ...
        <TouchableOpacity style={[styles.btnPrimary, !hasSelection && styles.btnDisabled]} onPress={handleContinue} activeOpacity={0.8}>
...
const styles = StyleSheet.create({
  ...
  topCurve: { paddingTop: 25, paddingBottom: 30, paddingHorizontal: 20, position: 'relative', overflow: 'hidden' },
  ...
  optionCard: { width: (SCREEN_WIDTH - 52) / 2, backgroundColor: '#ffffff', borderWidth: 1.5, borderColor: 'transparent', borderRadius: 20, paddingVertical: 24, paddingHorizontal: 16, alignItems: 'center', boxShadow: '0px 10px 25px rgba(0, 0, 0, 0.03)', elevation: 1 },
  optionCardSelected: { backgroundColor: 'rgba(59, 130, 246, 0.06)', borderColor: '#387bd5', elevation: 3 },
  ...
  btnPrimary: { backgroundColor: '#387bd5', height: 56, borderRadius: 30, alignItems: 'center', justifyContent: 'center', boxShadow: '0px 10px 25px rgba(59, 130, 246, 0.18)' },
