...
import { View, Text, TextInput, TouchableOpacity, StyleSheet, Animated, Image, Modal, ScrollView, Dimensions } from 'react-native';
...
const SOFT_ONBOARDING_STYLES = {
  colors: { primary: '#387bd5', primaryDark: '#1e5ab8', textMain: '#1a293b', textMuted: '#64748b', textSoft: '#94a3b8', border: '#e2e8f0', borderLight: 'rgba(56, 123, 213, 0.15)' },
...
export default function SoftOnboardingScreen() {
  ...
  const handleNameChange = (text: string) => {
    if (text.length <= 20) { setName(text); setErrorText(''); }
  };

  const handleContinue = async () => {
    const trimmed = name.trim();
    if (!trimmed) { setErrorText('Please enter your name'); return; }
    if (trimmed.length < 3) { setErrorText('Name must be at least 3 characters'); return; }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.topSection}>
        <LinearGradient colors={['#ffffff', '#e2effb', '#8db8f1']} locations={[0, 0.4, 1]} start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }} style={StyleSheet.absoluteFillObject} />
      ...
      <Animated.View style={[styles.bottomSection, slideFade(bottomAnim)]}>
        ...
        <TextInput style={[styles.inputField, errorText && name.trim().length < 3 && name.length > 0 && styles.inputError]} placeholder="Your name" placeholderTextColor={SOFT_ONBOARDING_STYLES.colors.textSoft} value={name} onChangeText={handleNameChange} autoCapitalize="words" maxLength={20} />
        <TouchableOpacity style={styles.consentRow} onPress={() => setShowTermsModal(true)} activeOpacity={0.7}>
        ...
        <TouchableOpacity style={[styles.btnPrimary, (!isNameValid || !consentChecked) && styles.btnDisabled]} onPress={handleContinue} activeOpacity={0.8}>
          ...
          <View style={styles.modalContent}>
            ...
            <ScrollView style={styles.modalBody}>
              <Text style={styles.modalBodyText}>
                ...
                By accessing and using the TeleHealings platform, you accept and agree to be bound by the terms and provision of this agreement.{'\n\n'}
            ...
            <View style={styles.modalActions}>
              <TouchableOpacity style={styles.btnReject} onPress={() => { setConsentChecked(false); setShowTermsModal(false); }}>
                <Text style={styles.btnRejectText}>Reject</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.btnAccept} onPress={() => { setConsentChecked(true); setShowTermsModal(false); setErrorText(''); }}>
...
const styles = StyleSheet.create({
  ...
  topSection: { paddingTop: SOFT_ONBOARDING_STYLES.spacing.topPadding, paddingBottom: 0, paddingHorizontal: 20, alignItems: 'center', position: 'relative', overflow: 'hidden' },
  ...
  inputField: { width: '100%', height: 56, paddingHorizontal: 24, borderWidth: 1, borderColor: SOFT_ONBOARDING_STYLES.colors.borderLight, borderRadius: 30, fontSize: 16, fontWeight: '500', color: SOFT_ONBOARDING_STYLES.colors.textMain, marginBottom: 24, backgroundColor: '#ffffff', boxShadow: '0px 4px 15px rgba(0, 0, 0, 0.02)', elevation: 2 },
  inputError: { borderColor: '#d93838' },
  consentRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 32 },
  checkbox: { width: 18, height: 18, borderWidth: 1.5, borderColor: '#cbd5e1', borderRadius: 6, alignItems: 'center', justifyContent: 'center', backgroundColor: '#ffffff', flexShrink: 0 },
  ...
  btnPrimary: { backgroundColor: SOFT_ONBOARDING_STYLES.colors.primary, height: 56, borderRadius: 30, alignItems: 'center', justifyContent: 'center', marginBottom: 15 },
