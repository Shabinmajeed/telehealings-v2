...
export default function MarketingScreen() {
  ...
  useEffect(() => {
    Animated.stagger(100, [
      ...
      Animated.timing(featuresAnim, { toValue: 1, duration: 600, useNativeDriver: false }),
      Animated.timing(statsAnim, { toValue: 1, duration: 600, useNativeDriver: false }),
      Animated.timing(bottomAnim, { toValue: 1, duration: 600, useNativeDriver: false }),
    ]).start();

    Animated.loop(
      Animated.sequence([
        Animated.timing(healiAnim, { toValue: 1, duration: 2000, useNativeDriver: false }),
        Animated.timing(healiAnim, { toValue: 0, duration: 2000, useNativeDriver: false }),
      ])
    ).start();
  }, []);
  ...
  const healiTranslate = healiAnim.interpolate({ inputRange: [0, 1], outputRange: [0, -8] });
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
      <View style={styles.topSection}>
        ...
        <Animated.View style={[styles.featuresList, slideFade(featuresAnim)]}>
          <View style={styles.featureItem}><Text style={styles.featureStar}>✦</Text><Text style={styles.featureText}>Ai-powered conversational partner</Text></View>
          <View style={styles.featureItem}><Text style={styles.featureStar}>✦</Text><Text style={styles.featureText}>Therapist handover continuity</Text></View>
          <View style={styles.featureItem}><Text style={styles.featureStar}>✦</Text><Text style={styles.featureText}>Self-help library</Text></View>
        </Animated.View>

        <Animated.View style={[styles.statsRow, slideFade(statsAnim)]}>
          <View style={styles.statBox}><Text style={styles.statValue}>200+</Text><Text style={styles.statLabel}>{'Verified\ntherapists'}</Text></View>
          <View style={styles.statBox}><Text style={styles.statValue}>8+</Text><Text style={styles.statLabel}>{'Languages\n'}</Text></View>
          <View style={styles.statBox}><Text style={styles.statValue}>1,000+</Text><Text style={styles.statLabel}>{'Hours of\ntherapy'}</Text></View>
        </Animated.View>

        <Svg height={30} width="100%" viewBox={`0 0 ${SCREEN_WIDTH} 30`} preserveAspectRatio="none" style={styles.curveSvg}>
          <Path d={`M0 30 L0 0 Q${SCREEN_WIDTH / 2} 30 ${SCREEN_WIDTH} 0 L${SCREEN_WIDTH} 30 Z`} fill="#ffffff" />
        </Svg>
      </View>
      ...
      <Animated.View style={[styles.bottomSection, slideFade(bottomAnim)]}>
        ...
        <View style={styles.ctaRow}>
          ...
          <TouchableOpacity style={styles.ctaButton} activeOpacity={0.8} onPress={() => router.push('/soft-onboarding')}>
...
const styles = StyleSheet.create({
  ...
  topSection: { paddingTop: 40, paddingBottom: 0, paddingHorizontal: 20, alignItems: 'center', position: 'relative', overflow: 'hidden' },
  ...
  statsRow: { flexDirection: 'row', gap: 12, width: '100%', maxWidth: 340, justifyContent: 'space-between', zIndex: 1, marginBottom: 30 },
  ...
  healiImage: { width: 130, height: 130, marginTop: 'auto', marginBottom: 10 },
