...
import {
  Search, Plus, X, CheckCircle, AlertTriangle
} from 'lucide-react';
  ...
  const handleSendChat = (text: string) => {
    setChats(prev => prev.map(c => {
      if (c.id === activeChatId) {
        return { ...c, messages: [...c.messages, { type: 'sent' as const, text, time: 'Just now' }], preview: text };
      }
      return c;
    }));
  };
    ...
    <div style={{ padding: '0 32px 16px', overflowY: 'auto', flex: 1 }}>
      <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', flexDirection: 'column', paddingBottom: 40 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 40, marginBottom: 12 }}>
          <h1 style={{ fontSize: 20, fontWeight: 700, color: '#0f172a', position: 'relative', padding: '0 0 8px 0', borderBottom: '2px solid #e2e8f0', margin: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            {healiInsightVisible && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 12px', background: 'linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%)', border: '1px solid #bfdbfe', borderRadius: 8, flexShrink: 0 }}>
                <div style={{ width: 24, height: 24, borderRadius: '50%', background: '#2563eb', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 2a7 7 0 0 1 7 7c0 2.38-1.19 4.47-3 5.74V17a2 2 0 0 1-2 2H10a2 2 0 0 1-2-2v-2.26C6.19 13.47 5 11.38 5 9a7 7 0 0 1 7-7z"/><path d="M9 21h6"/></svg>
                </div>
                ...
                <button onClick={() => setHealiInsightVisible(false)} style={{ background: 'none', border: 'none', color: '#93c5fd', cursor: 'pointer', padding: 2, borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 24, borderBottom: '1px solid #e2e8f0', marginBottom: 0 }}>
          <button onClick={() => setActiveTab('messages')} style={{ background: 'transparent', border: 'none', padding: '10px 4px', fontSize: 14, fontWeight: 600, cursor: 'pointer', position: 'relative', color: activeTab === 'messages' ? '#2a73d4' : '#64748b' }}>
            Messages{activeTab === 'messages' && <span style={{ position: 'absolute', bottom: -1, left: 0, right: 0, height: 2, background: '#2a73d4', borderRadius: '2px 2px 0 0' }} />}
          </button>
          <button onClick={() => setActiveTab('tickets')} style={{ background: 'transparent', border: 'none', padding: '10px 4px', fontSize: 14, fontWeight: 600, cursor: 'pointer', position: 'relative', color: activeTab === 'tickets' ? '#2a73d4' : '#64748b' }}>
            Support Tickets{activeTab === 'tickets' && <span style={{ position: 'absolute', bottom: -1, left: 0, right: 0, height: 2, background: '#2a73d4', borderRadius: '2px 2px 0 0' }} />}
          </button>
        </div>

        {/* ─── MESSAGES TAB ─── */}
        {activeTab === 'messages' && (
          <div style={{ display: 'flex', flex: 1, height: 'calc(100vh - 220px)', background: '#fff', borderRadius: 16, boxShadow: '0 4px 15px rgba(0,0,0,0.02)', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
            {/* Chat Sidebar */}
            <div style={{ width: 320, borderRight: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', background: '#f8fafc', flexShrink: 0 }}>
              <div style={{ padding: 16, borderBottom: '1px solid #e2e8f0' }}>
                <div style={{ position: 'relative', width: '100%' }}>
                  <svg style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', width: 14, height: 14, color: '#64748b', pointerEvents: 'none' }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                  <input type="text" placeholder="Search messages..." style={{ width: '100%', padding: '8px 10px 8px 32px', borderRadius: 8, border: '1px solid #e2e8f0', background: '#fff', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
                </div>
                <div style={{ display: 'flex', gap: 6, marginTop: 10, overflowX: 'auto' }}>
                  {['All', 'Open (1)', 'Pending (1)', 'Resolved (1)'].map(f => (
                    <button key={f} onClick={() => setChatFilter(f)} style={{ background: chatFilter === f ? '#eff6ff' : '#f1f5f9', border: `1px solid ${chatFilter === f ? '#bfdbfe' : 'transparent'}`, color: chatFilter === f ? '#2563eb' : '#475569', padding: '3px 10px', borderRadius: 99, fontSize: 11, fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap' }}>{f}</button>
                  ))}
                </div>
              </div>

              {/* Chat List using shared ConversationListItem */}
              <div style={{ flex: 1, overflowY: 'auto' }}>
                {filteredChats.map(chat => (
                  <ConversationListItem
                    ...
                    time={chat.time}
                    unread={chat.unread}
                    color={chat.escalated ? '#ef4444' : '#3b82f6'}
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: '#fff', minWidth: 0, height: '100%' }}>
              ...
              <div style={{ padding: '14px 20px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 36, height: 36, borderRadius: '50%', background: activeChat.escalated ? '#ef4444' : '#3b82f6', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, flexShrink: 0 }}>{activeChat.avatar}</div>
                  <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  {activeChat.escalated && (
                    <div style={{ background: '#fee2e2', color: '#991b1b', padding: '4px 10px', borderRadius: 6, fontSize: 11, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 2 }}>
                      <AlertTriangle size={12} /> Escalated
                    </div>
                  )}
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button style={{ background: '#fff', border: '1px solid #e2e8f0', color: '#0f172a', padding: '5px 10px', borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 2 }}><CheckCircle size={12} /> Resolve</button>
                    <button style={{ background: '#fff', border: '1px solid #fca5a5', color: '#dc2626', padding: '5px 10px', borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 2 }}><AlertTriangle size={12} /> Escalate</button>
                  </div>
                </div>
              </div>

              {/* Chat Messages using shared ChatBubble */}
              <div style={{ flex: 1, overflowY: 'auto', padding: 20, display: 'flex', flexDirection: 'column', gap: 14, background: '#fafafa', minHeight: 400 }}>
                {activeChat.messages.map((msg, i) => (
                  <ChatBubble key={i} type={msg.type === 'sent' ? 'sent' : 'received'} text={msg.text} time={msg.time} />
                ))}
              </div>

              {/* Chat Input using shared ChatInput */}
              <ChatInput onSend={handleSendChat} placeholder="Type a message..." />
            </div>
          </div>
        )}
          ...
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', margin: '12px 0', flexWrap: 'wrap', gap: 10 }}>
              <div style={{ position: 'relative', width: 220 }}>
                ...
                <input type="text" placeholder="Search tickets..." value={ticketSearch} onChange={(e) => setTicketSearch(e.target.value)} style={{ width: '100%', padding: '8px 12px 8px 34px', borderRadius: 99, border: '1px solid #e2e8f0', background: '#fff', fontSize: 13, outline: 'none', color: '#0f172a' }} />
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', border: '1px solid #e2e8f0', background: '#fff', color: '#64748b' }}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg> Export CSV
                </button>
                <button onClick={() => setShowTicketModal(true)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', border: 'none', background: '#2a73d4', color: '#fff' }}>
                  <Plus size={14} /> New Ticket
                </button>
              </div>
            </div>

            <div style={{ background: '#fff', borderRadius: 16, padding: '24px 32px', boxShadow: '0 4px 15px rgba(0,0,0,0.02)', display: 'flex', flexDirection: 'column', border: '1px solid #e2e8f0' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    {['Sl No', 'Ticket ID', 'Subject', 'User', 'Date', 'Priority', 'Status', ''].map((h, i) => (
                      <th key={i} style={{ textAlign: i === 7 ? 'center' : 'left', fontSize: 12, fontWeight: 500, color: '#64748b', padding: '8px 12px 8px 0', borderBottom: '1px solid #f1f5f9', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {filteredTickets.map((t) => {
                    ...
                    return (
                      <tr key={t.id}>
                        ...
                        <td style={{ padding: '10px 12px', borderBottom: '1px solid #f1f5f9' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            <div style={{ width: 32, height: 32, borderRadius: '50%', background: '#3b82f6', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, flexShrink: 0 }}>{getInitials(t.user)}</div>
                        ...
                        <td style={{ padding: '10px 12px', borderBottom: '1px solid #f1f5f9' }}>
                          <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: '3px 8px', borderRadius: 6, fontSize: 11, fontWeight: 600, minWidth: 80, textAlign: 'center', background: pc.bg, color: pc.text }}>{t.priority}</span>
                        </td>
                        <td style={{ padding: '10px 12px', borderBottom: '1px solid #f1f5f9' }}>
                          <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: '3px 8px', borderRadius: 6, fontSize: 11, fontWeight: 600, minWidth: 80, textAlign: 'center', background: sc.bg, color: sc.text }}>{t.status}</span>
                        </td>
                        <td style={{ padding: '10px 12px', borderBottom: '1px solid #f1f5f9', textAlign: 'center' }}>
                          <button style={{ background: '#eff6ff', color: '#2563eb', border: '1px solid #bfdbfe', borderRadius: 6, padding: '5px 10px', fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>View</button>
                        </td>
                      </tr>
                    );
                  })}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 16, borderTop: '1px solid #f1f5f9', marginTop: 16 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  {[1, 2, 3].map(p => (
                    <button key={p} style={{ width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 8, border: 'none', background: p === 1 ? '#2563eb' : '#f1f5f9', color: p === 1 ? '#fff' : '#334155', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>{p}</button>
                  ))}
                  <button style={{ width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 8, border: 'none', background: '#f1f5f9', color: '#334155', cursor: 'pointer' }}>
      ...
      {/* New Ticket Modal */}
      {showTicketModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15,23,42,0.4)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000 }} onClick={() => setShowTicketModal(false)}>
          <div style={{ background: '#fff', width: 450, maxWidth: '90%', borderRadius: 16, boxShadow: '0 20px 40px rgba(0,0,0,0.1)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
            <div style={{ padding: '18px 22px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#f8fafc' }}>
              ...
              <button onClick={() => setShowTicketModal(false)} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: '#64748b', padding: 4, borderRadius: '50%' }}><X size={20} /></button>
            </div>
